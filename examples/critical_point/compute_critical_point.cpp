...
#include "index_of_critical_point.h"
...
int main(int argc, char** argv)
{
    ...
    master.foreach([&](Block<real_t>* b, const diy::Master::ProxyWithLink& cp)
            {
                ...
                std::cout<<"shrink_ratio "<<shrink_ratio[0]<<" "<<shrink_ratio[1]<<" "<<shrink_ratio[2]<<" "<<shrink_ratio[3]<<std::endl;

                span_filter::compute_valid_span(sci_deriv_control_points,b,selected_span,shrink_ratio);
                ...
                [&](const tbb::blocked_range<size_t>& range)
                {
                    ...
                    for(auto i=range.begin();i!=range.end();++i)
                    {
                        ...
                        find_all_roots::newtonSolve(b,selected_span,root_temp_record[i],weights,i,original_root_size[i],filter_out,same_root_epsilon,block_itr_num,root_finding_epsilon,max_itr);
                ...
                // find_all_roots::convertFromDomain(domain_root[index],root[index],b->core_mins,local_domain_range);
                find_all_roots::getFunctionValue(b->mfa, b,domain_root[index],value[index]);


                find_all_roots::get_critical_point_index(b->mfa, b,domain_root[index],critical_point_type[index],HESSIAN_DET_EPSILON);

                // find_all_roots::getDerivative(b->mfa, b->vars[0].mfa_data,root[index],deriv_value[index]);
                // double result_gradient_average = find_all_roots::getAverageNorm(deriv_value[index]);
                ...
                // find_all_roots::convertToDomain(b->core_mins,local_domain_range,root_ori,root[index]);

                // find_all_roots::find_all_unique_root(root[index], test,same_root_epsilon,root_duplicated_number[index]);
                ...
                std::cout<<itr_num.load()<<" "<<initial_point_num*selected_span[0].size()<<std::endl;
    ...
    std::vector<MatrixXd> root_matrix(domain_root.size());
    int domain_size, value_size;
    for(int i=0;i<domain_root.size();i++)
    {
        domain_size = domain_root[i][0].size();
        value_size = value[i][0].size();
        root_matrix[i].resize(domain_root[i].size(),domain_size+value_size+1);
        for(int j=0;j<domain_root[i].size();j++)
        {
            root_matrix[i].block(j,0,1,domain_size) = domain_root[i][j].transpose();
            root_matrix[i].block(j,domain_size,1,value_size) = value[i][j].transpose();
            root_matrix[i](j,domain_size+value_size) = (double)critical_point_type[i][j];
        }
    }
